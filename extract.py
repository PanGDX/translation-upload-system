...
def extract_and_replace_names(translation_prompt, reduction_prompt, extraction_prompt):
    ...
    try:
        for chapter in range(from_chapter, to_chapter + 1):
            ...
            potential_names = [
                f"{current_chapter_string.zfill(i)}.txt" for i in range(0, 6)]
        ...
        finalNameList = sorted(list(set(finalNameList)), key=len)
        ...
        gpt_translated_dict = {line.split(
            "-")[0].strip(): line.split("-")[1].strip() for line in final_namelist if line}

        argos_translated = {name: translate_text(
            name) for name in gpt_translated_dict}
        ...
        try:
            for counter, name in enumerate(argos_translated, 1):
                gpt_name = gpt_translated_dict.get(
                    name, "GPT Translation encountered an error/missing this name")
